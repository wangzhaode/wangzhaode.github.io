...
1.  **因果卷积 (局部信息)**: 拼接后的 `qkv` 张量经过一个一维因果卷积，它像一个滑动窗口，用于高效捕捉每个 Token 与其附近邻居之间的局部上下文。
    ...
    ![](https://gitee.com/wangzhaode/asset/raw/main-md2zhihu-asset@main-md2zhihu-asset/qwen3-next/pythonmodeling_qwen3_nextpy-clas-d2336559f9bf7e17.jpg)
...
1.  **门控信号生成**: 模型从投影 `a` 和 `b` 中，学习两个关键的、类似 RNN 的门控信号：
    ...
    ![](https://gitee.com/wangzhaode/asset/raw/main-md2zhihu-asset@main-md2zhihu-asset/qwen3-next/pythonbeta=bsigmoidg=-selfA_logf-6820b36c26126507.jpg)

1.  **门控增量规则 (全局信息)**: 将 `q, k, v` 和门控信号 `beta, g` 送入核心的递归函数，进行全局信息传递。这一步是实现 O(N) 复杂度的关键。

    ![](https://gitee.com/wangzhaode/asset/raw/main-md2zhihu-asset@main-md2zhihu-asset/qwen3-next/pythoncore_attn_out_=selfchunk_g-a2255d060b279e6a.jpg)

1.  **输出门控**: `GatedDeltaNet` 的输出 `core_attn_out` 还会被 `z` 进行最终的门控调制，然后才传递给下一层。

    ![](https://gitee.com/wangzhaode/asset/raw/main-md2zhihu-asset@main-md2zhihu-asset/qwen3-next/pythoncore_attn_out=selfnormcore-a31f9d16ecca41cc.jpg)
...
![](https://gitee.com/wangzhaode/asset/raw/main-md2zhihu-asset@main-md2zhihu-asset/qwen3-next/pythonmodeling_qwen3_nextpy-clas-69bd19fe3ffb96a4.jpg)
